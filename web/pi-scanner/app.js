...
(function installP25NoPageAutostartFetchGuard() {
  ...
  window.fetch = function guardedP25Fetch(input, init) {
    ...
    const scannerStart = method === 'POST' && url.includes('/pi-scanner/api/scanner/start');
...
function audioStreamUrl() { return `/pi-scanner/audio-api/audio.wav`; }
function testToneUrl() { return `/pi-scanner/audio-api/test-tone.wav`; }
...
async function refreshStatus() {
  try {
    latestStatus = await fetchJson('/pi-scanner/api/status');
...
async function refreshConfig() {
  try {
    const response = await fetchJson('/pi-scanner/api/config');
...
async function startScannerAndAudio() {
  ...
  try {
    const status = await postJson('/pi-scanner/api/scanner/start');
...
async function stopScanner() {
  ...
  try {
    const status = await postJson('/pi-scanner/api/scanner/stop');
    renderDashboard(status);
...
async function refreshProfiles() {
  try {
    const payload = await fetchJson('/pi-scanner/api/config/named');
...
async function loadSelectedProfile() {
  ...
  try {
    const payload = await postJson('/pi-scanner/api/config/named/load', { id, apply: true });
...
async function saveCurrentProfile() {
  ...
  try {
    const payload = await postJson('/pi-scanner/api/config/named/save', { name, apply: false });
...
async function deleteSelectedProfile() {
  ...
  if (!id) { setText('profileStatusText', 'Select a profile first.'); return; }
  try {
    await postJson('/pi-scanner/api/config/named/delete', { id });
...
async function refreshRadioReferenceStatus() {
  try { renderRadioReferenceStatus(await fetchJson('/pi-scanner/api/radioreference/status')); }
...
async function saveRadioReferenceCredentials() {
  ...
  try {
    const status = await postJson('/pi-scanner/api/radioreference/save-credentials', payload);
...
async function testRadioReferenceLogin() {
  try {
    const result = await postJson('/pi-scanner/api/radioreference/test-login');
...
async function callSystemsEndpoint(payload) {
  ...
  try { return await fetchJson(`/pi-scanner/api/radioreference/systems?${params.toString()}`); }
  catch (_first) { return postJson('/pi-scanner/api/radioreference/systems', payload); }
}
...
async function callSitesEndpoint(systemId) {
  ...
  return postJson('/pi-scanner/api/radioreference/sites', {
    system_id: systemId,
    sid: systemId,
    state,
    county,
...
async function findRrSystems() {
  ...
  try {
    ...
    const result = await postJson('/pi-scanner/api/radioreference/import', payload);
...
async function importAndSaveRr() {
  ...
  try {
    ...
    if (result.ok && result.config) {
      try {
        const saved = await postJson('/pi-scanner/api/config/named/save', { name: payload.name, config: result.config, apply: true });
...
async function autoCalibratePpm() {
  ...
  try {
    const result = await postJson('/pi-scanner/api/calibration/ppm/run', { span_ppm: 3, step_ppm: 1, dwell_seconds: 8, apply_voice: false });
...
async function saveImportedProfile(name) {
  const result = await postJson('/pi-scanner/api/config/named/save', { name, apply: false });
  await refreshProfiles();
...
async function refreshAnalogChannels() {
  try {
    const result = await fetchJson('/pi-scanner/api/analog/channels');
...
async function importAnalogCsv() {
  ...
  try {
    ...
    const result = await postJson('/pi-scanner/api/analog/channels/import', {
      filename: file.name,
...
async function importP25CsvFile() {
  ...
  try {
    const name = profileNameForFile(file);
    const result = await postJson('/pi-scanner/api/p25/csv/import', {
      filename: file.name,
      csv_text: await file.text(),
...
async function exportSelectedProfile(kind) {
  ...
  try {
    const result = await postJson('/pi-scanner/api/config/named/export', { id, kind });
...
(() => {
  ...
  window.fetch = async (...args) => {
    ...
    if (!requestUrl.includes("/pi-scanner/api/radioreference/sites")) {
      return response;
    }
...
(function installAnalogLockCounters() {
  ...
  async function refreshAnalogLockCounters() {
    try {
      const response = await fetch("/pi-scanner/api/analog/status", {
        cache: "no-store",
...
(function installAccurateScannerStatus() {
  ...
  async function refreshAccurateStatus() {
    try {
      const backendResponse = await fetch('/pi-scanner/api/status', {cache:'no-store', credentials:'same-origin'});
...
(function installAnalogChannelCenterDisplay() {
  ...
  async function refreshAnalogCenterDisplay() {
    ...
    try {
      ...
      const analogStatus = await fetchJson('/pi-scanner/api/analog/status');
...
(function installAnalogSquelchValueLayoutV114() {
  ...
  async function postAnalogAction(role, action) {
    return postJson('/pi-scanner/api/analog/control', {
      role,
      action,
    });
  }
