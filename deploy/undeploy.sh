...
REPORT_SERVICE_FILE='/etc/systemd/system/n0jcg-admin-report.service'
...
printf '%s\n' "${ROC_SUDO_PASS}" | "${SSH[@]}" "${ROC_USER}@${ROC_HOST}" "sudo -S -p '' systemctl disable --now n0jcg-roc.service n0jcg-operator-helper.service n0jcg-admin-report.service"
if [[ "${1:-}" == '--stop-only' ]]; then
...
printf '%s\n' "${ROC_SUDO_PASS}" | "${SSH[@]}" "${ROC_USER}@${ROC_HOST}" "sudo -S -p '' rm -f '${SERVICE_FILE}' '${HELPER_SERVICE_FILE}' '${REPORT_SERVICE_FILE}' '${HELPER_FILE}' /etc/n0jcg/operator-controls.env /etc/n0jcg/admin-report.env && sudo systemctl daemon-reload"
