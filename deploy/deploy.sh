...
printf '%s\n' "${ROC_SUDO_PASS}" | "${SSH[@]}" "${ROC_USER}@${ROC_HOST}" "sudo -S -p '' sh -c 'install -d -o n0jcg -g n0jcg -m 0755 /var/lib/n0jcg-roc && if [ -e /var/lib/n0jcg-roc/cwop.json ]; then chown n0jcg:n0jcg /var/lib/n0jcg-roc/cwop.json; fi && install -m 0644 ${ROC_REMOTE_DIR}/deploy/n0jcg-roc.service /etc/systemd/system/n0jcg-roc.service && systemctl daemon-reload && systemctl enable n0jcg-roc.service && systemctl restart n0jcg-roc.service'"
